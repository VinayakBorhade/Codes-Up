...
<div class="container">
	...
	<table class="table" >
		...
		$userid=$_SESSION['userid'];
			$sql="select * from following where follower_id='$userid' ";
			$result = mysqli_query($link, $sql);
			if (mysqli_num_rows($result) > 0) {
				// output data of each row
				while($row = mysqli_fetch_assoc($result)) {
					$userid_key=$row['user_id'];
					$sql1="select * from users where id='$userid_key' ";
										...
										echo "<td>";
											echo "<div class='media'>";
												...
												echo "<div class='media-body'>
													...
													$userid2=$row1['id'];
													$sql2="select * from following where user_id='$userid2' and follower_id='$userid' ";
													$result2 = mysqli_query($link, $sql2);
													if(mysqli_num_rows($result2) == 1){
														echo "<button type='button' onclick=\"location.href='action.php?id=$userid2&do=unfollow'\" class='btn btn-success btn-filter' data-target='pagado'>Unfollow</button></span>";
													}else{
														echo "<button type='button' onclick=\"location.href='action.php?id=$userid2&do=follow'\" class='btn btn-success btn-filter' data-target='pagado'>Follow</button></span>";
													}
													/*if (mysqli_num_rows($result2) > 0) {
														while($row2 = mysqli_fetch_assoc($result2)){
															if($row2['follower_id']==$_SESSION['username']){
																// user follows this row1['username'] 
																echo "<button type='button' onclick=\"location.href='action.php?id=$userid2&do=unfollow'\" class='btn btn-success btn-filter' data-target='pagado'>Unfollow</button></span>";
																$flag=true;break;
															}
														}
													...
													echo "<h4 class='title'><a href='grid.php?Other_User_name=". $row1['username'] ."'>
														".$row1['username']."</a>
													</h4>
													<p class='summary'>".$row1['id']."</p>
												</div>";
											echo "</div>
										</td>
									</tr>";
